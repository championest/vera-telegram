...
export const toolDefinitions: FunctionDeclaration[] = [
  ...
  // ─── Web automation (Playwright) ───
  {
    ...
    parameters: {
      type: 'OBJECT' as any,
      properties: {
        url: { type: 'STRING' as any, description: 'URL ที่จะเปิด (เติม https:// ให้อัตโนมัติถ้าไม่มี)' },
      },
      required: ['url'],
    },
  },
  {
    ...
    parameters: {
      type: 'OBJECT' as any,
      properties: {
        target: { type: 'STRING' as any, description: 'Visible text บนปุ่ม/ลิงก์ หรือ CSS selector (เริ่ม #, ., [ จะถือว่าเป็น selector)' },
        role: { type: 'STRING' as any, description: 'Optional ARIA role เช่น "button", "link" เพื่อช่วยเลือก element ให้แม่นขึ้น' },
      },
      required: ['target'],
    },
  },
  {
    ...
    parameters: {
      type: 'OBJECT' as any,
      properties: {
        target: { type: 'STRING' as any, description: 'Label, placeholder หรือ CSS selector ของ input' },
        value: { type: 'STRING' as any, description: 'ค่าที่จะกรอก' },
        submit: { type: 'BOOLEAN' as any, description: 'true = กด Enter หลังกรอก. Default false.' },
      },
  ...
  // ─── Champ vision ───
  {
    ...
    parameters: {
      type: 'OBJECT' as any,
      properties: {
        title: { type: 'STRING' as any, description: 'ชื่อ vision เช่น "มีบ้านหลังใหม่"' },
        body: { type: 'STRING' as any, description: 'รายละเอียด vision' },
        category: {
          type: 'STRING' as any,
        ...
        image_url: { type: 'STRING' as any, description: 'URL รูปภาพประกอบ (optional)' },
      },
      required: ['title'],
    },
  },
  // ─── Champ finance ───
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        label: { type: 'STRING' as any, description: 'ชื่อรายการ เช่น "เงินเดือน", "ค่าเช่า", "เก็บเงินซื้อคอนโด"' },
        amount: { type: 'NUMBER' as any, description: 'จำนวนเงิน (บาท)' },
        category: { type: 'STRING' as any, description: 'หมวดหมู่ เช่น salary, rent, food, investment' },
        month: { type: 'STRING' as any, description: 'เดือนที่บันทึก เช่น "2026-05". Default: เดือนปัจจุบัน' },
  ...
  // ─── Champ calendar sync ───
  {
    ...
    parameters: {
      type: 'OBJECT' as any,
      properties: {
        days: { type: 'NUMBER' as any, description: 'จำนวนวันข้างหน้าที่จะดึง. Default: 14' },
      },
      required: [],
    },
  },
    ...
    parameters: {
      ...
      properties: {
        task: {
          type: 'STRING' as any,
          description:
            'คำสั่งงานละเอียดพอที่ Claude Code จะทำได้โดยไม่ต้องถามกลับ (ไทยหรืออังกฤษ) ระบุสิ่งที่ต้องการให้ชัด',
        },
        project: {
          type: 'STRING' as any,
          description:
            'โฟลเดอร์ project ใต้ ~/Projects เช่น "up-level-guild-members-web", "UpLevelKids", "team-dashboard", "up-level-leaderboard", "vera-telegram" หรือ alias: guild, kids, marketplace, lorcana, dashboard, poster, hq, tpt. เว้นว่างถ้าไม่แน่ใจ',
        },
      },
      required: ['task'],
    },
  },
  {
    ...
    description:
      'เช็คสถานะ/ผลของงานที่เคยสั่ง Claude Code ผ่าน dispatch_claude_task และดูว่าเครื่อง Mac ออนไลน์ไหม. ใช้เมื่อ Champ ถาม "งานเสร็จยัง", "เครื่องออนไลน์ไหม", "ผลเป็นไง"',
    parameters: {
      type: 'OBJECT' as any,
      properties: {
        limit: { type: 'NUMBER' as any, description: 'จำนวนงานล่าสุดที่จะแสดง. Default: 5' },
      },
      required: [],
    },
  },
  // ─── Champ personal tasks ───
  {
    ...
    parameters: {
      type: 'OBJECT' as any,
      properties: {
        title: { type: 'STRING' as any, description: 'ชื่องาน (Thai or English)' },
        status: {
          type: 'STRING' as any,
        ...
        notes: { type: 'STRING' as any, description: 'รายละเอียดเพิ่มเติม' },
        due_date: { type: 'STRING' as any, description: 'วันกำหนดส่ง เช่น "2026-06-01" หรือ "สิ้นเดือนนี้"' },
        tags: {
          type: 'ARRAY' as any,
          items: { type: 'STRING' as any },
  ...
  },
  {
    ...
    parameters: {
      type: 'OBJECT' as any,
      properties: {
        task_id: { type: 'STRING' as any, description: 'Firestore document ID ของงานที่ต้องการอัปเดต' },
        status: {
          type: 'STRING' as any,
          enum: ['TODO', 'IN_PROGRESS', 'DONE', 'BLOCKED'],
        ...
        notes: { type: 'STRING' as any, description: 'Notes ใหม่' },
        due_date: { type: 'STRING' as any, description: 'วันกำหนดส่งใหม่' },
      },
      required: ['task_id'],
    },
  },
