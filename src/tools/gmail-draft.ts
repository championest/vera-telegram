...
export async function gmailCreateDraft(args: Record<string, unknown>): Promise<string> {
  ...
  if (!subject || !body) return 'กรุณาระบุ subject และ body ค่ะ';
...
export async function gmailListDrafts(args: Record<string, unknown>): Promise<string> {
  if (!await isConnected()) return NOT_CONNECTED;
  const auth = await getAuthedClient();
  if (!auth) return NOT_CONNECTED;
  ...
  if (drafts.length === 0) return 'ไม่มี draft ค่ะ';

  const details = await Promise.all(
    drafts.map(d => gmail.users.drafts.get({ userId: 'me', id: d.id!, format: 'metadata',
  ...
  const lines = details.map(d => {
    ...
    const to = headers.find((h: any) => h.name === 'To')?.value ?? '(ยังไม่ระบุ)';
    return `• *${subject}*\n  ถึง: ${to}\n  ID: \`${d.data.id}\``;
  });
