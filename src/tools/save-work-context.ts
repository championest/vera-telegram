...
export async function saveWorkContext(input: Record<string, unknown>): Promise<string> {
  const topic = String(input['topic'] ?? '').trim();
  const summary = String(input['summary'] ?? '').trim();
  if (!topic || !summary) return 'ต้องระบุ topic และ summary';

  const actionItems = Array.isArray(input['action_items'])
    ? (input['action_items'] as unknown[]).map(String)
    : [];
  ...
  if (actionItems.length) noteLines.push(`Actions: ${actionItems.join(', ')}`);
  if (input['project']) noteLines.push(`Project: ${input['project']}`);
...
export async function getVeraConversations(input: Record<string, unknown>): Promise<string> {
  const limit = Number(input['limit'] ?? 5);
  const unsyncedOnly = input['unsynced_only'] === true;

  let query = db.collection('vera-conversations').orderBy('timestamp', 'desc');
  if (unsyncedOnly) query = query.where('synced_to_claude', '==', false) as any;
  ...
  if (!snap.empty) {
    ...
    for (const doc of snap.docs) {
      ...
      const ts = (d['timestamp'] as any)?.toDate?.()?.toLocaleDateString('th-TH') ?? '';
  ...
  try {
    ...
    if (claudeRecent.length) {
      ...
      for (const entry of claudeRecent) {
        ...
        if (entry.user_msg) lines.push(`   You: ${entry.user_msg}`);
        if (entry.claude_summary) lines.push(`   Claude: ${entry.claude_summary}`);
        lines.push('');
      }
    }
  } catch (err) {
    console.warn('[get_vera_conversations] claude context merge failed:', (err as Error)?.message);
  }

  if (!lines.length) return 'ไม่มี work context จาก Vera หรือ Claude';
  return lines.join('\n');
}
