...
export async function googleDriveSave(args: Record<string, unknown>): Promise<string> {
  const title = String(args.title ?? '');
  const content = String(args.content ?? '');
  const folderName = String(args.folder ?? 'Vera Research');

  if (!title || !content) return 'กรุณาระบุ title และ content';

  const auth = await getAuthedClient();
  if (!auth) return 'Google ยังไม่ได้เชื่อมค่ะ — กรุณาใช้ /connect';
