...
export async function webSearch(args: Record<string, unknown>): Promise<string> {
  ...
  if (!apiKey) return 'web_search ไม่พร้อมใช้งาน — กรุณาตั้งค่า TAVILY_API_KEY ใน Railway';
  ...
  if (!query) return 'กรุณาระบุ query ที่ต้องการค้นหา';
  ...
  const lines: string[] = [`🔍 ค้นหา: "${data.query}"\n`];
...
export async function fetchUrl(args: Record<string, unknown>): Promise<string> {
  const apiKey = config.TAVILY_API_KEY;
  if (!apiKey) return 'fetch_url ไม่พร้อมใช้งาน — กรุณาตั้งค่า TAVILY_API_KEY ใน Railway';

  const url = String(args.url ?? '');
  if (!url) return 'กรุณาระบุ URL';
  ...
  if (!result) return `ไม่สามารถดึงเนื้อหาจาก ${url}`;
