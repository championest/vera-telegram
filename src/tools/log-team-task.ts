...
function normalizeMember(raw: unknown): string {
...
export async function logTeamTask(input: Record<string, unknown>, userId: string): Promise<string> {
  ...
  if (taskId) {
    ...
    const updates: Record<string, unknown> = { updatedAt: FieldValue.serverTimestamp(), updatedBy: userId };
    if (input['status'])     updates['status'] = input['status'];
    if (input['notes'])      updates['notes'] = input['notes'];
    if (input['handoff_to']) updates['handoff_to'] = input['handoff_to'];
