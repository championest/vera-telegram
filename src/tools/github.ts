...
export async function githubListRepos(args: Record<string, unknown>): Promise<string> {
  if (!config.GITHUB_TOKEN) return 'github tools ไม่พร้อม — ตั้งค่า GITHUB_TOKEN ใน Railway';
  ...
  const repos = (await res.json()) as { full_name: string; description: string | null; default_branch: string }[];
  return `📦 Repos (${repos.length}):\n` + repos.map(r => `• ${r.full_name} (${r.default_branch}) — ${r.description ?? 'no description'}`).join('\n');
}

export async function githubReadFile(args: Record<string, unknown>): Promise<string> {
  if (!config.GITHUB_TOKEN) return 'github tools ไม่พร้อม — ตั้งค่า GITHUB_TOKEN ใน Railway';
  const repo = String(args.repo ?? '');
  const path = String(args.path ?? '');
  const branch = String(args.branch ?? 'main');
  if (!repo || !path) return 'ต้องระบุ repo และ path';
...
export async function githubUpdateFile(args: Record<string, unknown>): Promise<string> {
  if (!config.GITHUB_TOKEN) return 'github tools ไม่พร้อม — ตั้งค่า GITHUB_TOKEN ใน Railway';
  const repo = String(args.repo ?? '');
  const path = String(args.path ?? '');
  ...
  const branch = String(args.branch ?? 'main');
  if (!repo || !path || !content) return 'ต้องระบุ repo, path, และ content';
...
export async function githubCreatePr(args: Record<string, unknown>): Promise<string> {
  if (!config.GITHUB_TOKEN) return 'github tools ไม่พร้อม — ตั้งค่า GITHUB_TOKEN ใน Railway';
  const repo = String(args.repo ?? '');
  ...
  if (!repo || !title || !head) return 'ต้องระบุ repo, title, head';
...
export async function githubListPrs(args: Record<string, unknown>): Promise<string> {
  if (!config.GITHUB_TOKEN) return 'github tools ไม่พร้อม — ตั้งค่า GITHUB_TOKEN ใน Railway';
  const repo = String(args.repo ?? '');
  const state = String(args.state ?? 'open');
  if (!repo) return 'ต้องระบุ repo';
  ...
  if (!prs.length) return `ไม่มี PR (${state}) ใน ${repo}`;
