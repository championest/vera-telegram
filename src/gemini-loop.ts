...
export interface LoopOptions {
  ...
  /** Optional inline file parts (PDF base64, etc.) attached to the user turn */
  attachments?: Part[];
  /** Skip appending the user message to memory (caller already did it) */
  skipAppend?: boolean;
}

export async function runGeminiLoop(opts: LoopOptions, modelName?: string): Promise<string> {
  ...
  for (let round = 0; round < 12; round++) {
    ...
    const functionResponses: Part[] = await Promise.all(
      calls.map(async (call) => {
        const output = await executeToolCall(call.name, (call.args ?? {}) as Record<string, unknown>, userId);
        if (sendUpdate && ['save_research', 'google_drive_save', 'notebooklm_create'].includes(call.name)) {
          try { await sendUpdate(`✅ ${TOOL_LABELS[call.name] ?? call.name} เสร็จ`); } catch { /* non-fatal */ }
        }
        return {
          functionResponse: {
            name: call.name,
