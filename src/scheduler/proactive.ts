...
async function send(bot: Bot, text: string): Promise<void> {
  try {
    ...
    try { await bot.api.sendMessage(CHAT_ID, text); } catch { /* drop */ }
  }
}

// ───────────────── Email scan ─────────────────
async function scanImportantEmail(bot: Bot): Promise<void> {
  ...
  const fresh: { id: string; from: string; subject: string; snippet: string }[] = [];
...
// ───────────────── Calendar pre-alert ─────────────────
async function scanUpcomingEvents(bot: Bot): Promise<void> {
  if (!await isConnected()) return;
  const auth = await getAuthedClient();
  if (!auth) return;
...
// ───────────────── Overdue tasks (daily) ─────────────────
async function scanOverdueTasks(bot: Bot): Promise<void> {
  const today = new Date().toLocaleDateString('sv-SE', { timeZone: 'Asia/Bangkok' });
  const state = await getState();
  if (state.lastOverdueScanDay === today) return; // already scanned today
...
// ───────────────── Boot ─────────────────
export function startProactiveScheduler(bot: Bot): void {
  // Calendar pre-alert — every 5 minutes
  cron.schedule('*/5 * * * *', async () => {
    if (!await isProactiveEnabled()) return;
    try { await scanUpcomingEvents(bot); }
    catch (err) { console.error('[proactive/calendar]', err); }
  });

  // Important email — every 10 minutes
  cron.schedule('*/10 * * * *', async () => {
    if (!await isProactiveEnabled()) return;
    try { await scanImportantEmail(bot); }
    catch (err) { console.error('[proactive/email]', err); }
  });

  // Overdue tasks — 08:30 BKK (= 01:30 UTC)
  cron.schedule('30 1 * * *', async () => {
    if (!await isProactiveEnabled()) return;
    try { await scanOverdueTasks(bot); }
    catch (err) { console.error('[proactive/overdue]', err); }
  });
