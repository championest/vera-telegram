...
export async function sendPosterDojoDigest(bot: Bot): Promise<number> {
  ...
  const n = snap.size;
  if (n === 0) return 0; // เงียบไว้ถ้าวันนี้ไม่มีงาน

  await bot.api.sendMessage(CHAT_ID, `🥋 วันนี้ Dojo ทำโพสไป ${n} งาน`);
  return n;
}

export function startPosterDojoDigestScheduler(bot: Bot): void {
  // 21:00 Asia/Bangkok daily
  cron.schedule(
    '0 21 * * *',
    () => {
      sendPosterDojoDigest(bot).catch((err) =>
        console.error('[poster-dojo-digest] error:', err),
      );
    },
