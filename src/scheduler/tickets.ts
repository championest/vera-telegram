...
function buildClaudeTaskPrompt(
    type: string,
    source: string,
    title: string,
    description: string,
    ...
    lines.push(
        `Goal: diagnose, fix in code, commit, deploy if appropriate, then mark this ticket resolved by updating tickets/${ticketId} { status: "resolved" }.`,
    );
    return lines.join('\n');
}
...
async function notifyAndDispatch(bot: Bot, doc: FirebaseFirestore.QueryDocumentSnapshot) {
    ...
    try {
        ...
        await bot.api.sendMessage(
            CHAT_ID,
            `${emoji} ${type.toUpperCase()} [${source}] — auto-dispatched\n${title}\n${description}\n${url}\nticket: ${doc.id}\ntask: ${taskRef.id}`,
            {
                reply_markup: {
                    inline_keyboard: [[
                        { text: '✋ Cancel', callback_data: `tk:cancel:${doc.id}` },
