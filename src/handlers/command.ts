...
export async function handleCode(ctx: Context) {
  ...
  try {
    ...
    await ctx.reply(`เชื่อมไม่สำเร็จค่ะ — ลอง /connect ใหม่อีกครั้งนะคะ${hint}`, { parse_mode: 'Markdown' }).catch(() =>
      ctx.reply(`เชื่อมไม่สำเร็จค่ะ — ลอง /connect ใหม่อีกครั้งนะคะ`)
    );
  }
}
