...
export const DEFAULT_PROVIDER: ModelProvider = 'auto';
...
export interface AgentOptions {
  ...
  /** Override stored preference for this call only */
  provider?: ModelProvider;
  /** Override Claude model (e.g. claude-haiku-4-5 for chat, claude-sonnet-4-6 for dispatch) */
  claudeModel?: string;
}

const PREFS_COLLECTION = 'vera-prefs';
const prefCache = new Map<string, { value: ModelProvider; expiresAt: number }>();
...
export async function getUserProvider(userId: string): Promise<ModelProvider> {
  ...
  try {
    ...
    const value: ModelProvider = stored === 'claude' || stored === 'gemini' || stored === 'auto' ? stored : DEFAULT_PROVIDER;
...
export async function setUserProvider(userId: string, provider: ModelProvider): Promise<void> {
...
function isOverloaded(err: any): boolean {
  ...
  if (typeof status === 'number' && (status === 429 || status === 503 || status === 529 || status >= 500)) return true;
...
function isClaudeHardFail(err: any): boolean {
  const msg = String(err?.message ?? err?.error?.error?.message ?? err ?? '');
  const status = err?.status ?? err?.statusCode;
  if (status === 401 || status === 403) return true;
  if (status === 400 && /credit|billing|balance|too low|quota|insufficient/i.test(msg)) return true;
...
function filesToClaudeBlocks(files: NormalizedFile[]): Anthropic.ContentBlockParam[] {
  ...
  for (const f of files) {
    ...
    if (mime.startsWith('image/')) {
      blocks.push({
        type: 'image',
        source: { type: 'base64', media_type: mime as any, data: f.buffer.toString('base64') },
      });
...
function filesToGeminiParts(files: NormalizedFile[]): Part[] {
  const parts: Part[] = [];
  for (const f of files) {
    const mime = f.mimeType;
...
export async function runAgent(opts: AgentOptions): Promise<string> {
  ...
  const provider: ModelProvider = HAS_CLAUDE
  ...
  const runClaude = (): Promise<string> => runClaudeLoop({
    userId: opts.userId,
  ...
  const runGemini = (model?: string): Promise<string> => runGeminiLoop({
    userId: opts.userId,
    userText: opts.userText,
    onProgress: opts.onProgress,
    sendUpdate: opts.sendUpdate,
  ...
  const runGeminiChain = async (): Promise<string> => {
    try {
  ...
  if (provider === 'gemini') return runGeminiChain();
  ...
  if (Date.now() < claudeCooldownUntil) return runGeminiChain();

  try {
    return await runClaude();
  } catch (err) {
    const hard = isClaudeHardFail(err);
    if (!hard && !isOverloaded(err)) throw err;
    if (hard) {
