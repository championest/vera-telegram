...
export async function friendlyGoogleError(
  e: unknown,
  api: string,
  apiId: string,
): Promise<string | null> {
  ...
  if (isApiDisabled(e)) {
    return (
      `Google ${api} API ยังไม่ได้เปิดใช้งานใน Cloud project ค่ะ ⚙️\n\n` +
