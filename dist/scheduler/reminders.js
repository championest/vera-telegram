    ...
    cron.schedule('* * * * *', async () => {
        ...
        try {
            ...
            for (const doc of snap.docs) {
                ...
                try {
                    await bot.api.sendMessage(CHAT_ID, `⏰ *Reminder*\n${reminder['message']}`, {
                        parse_mode: 'Markdown',
                        reply_markup: {
                            inline_keyboard: [[
                    ...
                    if (reminder['repeat'] === 'daily') {
                        const next = reminder['remindAt'].toDate();
...
export async function handleReminderCallback(bot, action, docId, callbackQueryId) {
