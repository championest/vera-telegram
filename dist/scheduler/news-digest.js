...
export async function sendNewsDigest(bot) {
    const snap = await db
    ...
    const n = snap.size;
    if (n === 0)
        return 0; // เงียบไว้ถ้าไม่มีข่าวรอยืนยัน
    await bot.api.sendMessage(CHAT_ID, `📰 วันนี้มีข่าว ${n} ชิ้นที่แอดมินยอมรับแล้ว รอคุณยืนยันโพส → uplevelguild.com/news-admin`);
    return n;
}
export function startNewsDigestScheduler(bot) {
    // 20:00 Asia/Bangkok daily
    cron.schedule('0 20 * * *', () => {
        sendNewsDigest(bot).catch((err) => console.error('[news-digest] error:', err));
