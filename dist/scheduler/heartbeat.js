...
export function startHeartbeat(host = 'mini') {
    const write = () => DOC.set({ ts: Date.now(), host }, { merge: true }).catch((e) => console.error('[heartbeat] write failed', e?.message ?? e));
    write();
 ...
 */
export async function isPrimaryStale(staleMs = STALE_MS) {
    try {
        ...
        console.error('[heartbeat] read failed — staying idle', e?.message ?? e);
        return false;
    }
}
