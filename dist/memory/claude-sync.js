...
const buffer = [];
let hydrated = false;
export async function recordClaudeSync(entry) {
    ...
    try {
        ...
        console.warn('[claude-sync] firestore write failed:', err?.message);
    }
}
export async function getRecentClaudeContext(limit = 7) {
    if (!hydrated) {
...
async function hydrateFromFirestore() {
    try {
        ...
        console.warn('[claude-sync] hydrate failed:', err?.message);
    }
}
