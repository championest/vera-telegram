...
export async function dispatchClaudeTask(input) {
    const task = String(input['task'] ?? '').trim();
    if (!task)
        return 'ต้องระบุ task ที่จะให้ Claude Code ทำค่ะ';
...
export async function checkClaudeTasks(input) {
    ...
    if (snap.empty)
        return 'ยังไม่มีงานที่เคยสั่งผ่าน remote executor ค่ะ';
    const online = await executorOnline();
    const lines = [`*Remote tasks ล่าสุด* (เครื่อง Mac: ${online ? '🟢 online' : '🔴 offline'})\n`];
    for (const d of snap.docs) {
        const t = d.data();
        const emoji = t['status'] === 'done' ? '✅' : t['status'] === 'failed' ? '❌' : t['status'] === 'running' ? '⚙️' : '⏳';
