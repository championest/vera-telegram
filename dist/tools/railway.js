...
export async function railwayListServices(args) {
    if (!config.RAILWAY_TOKEN)
        return 'railway tools ไม่พร้อม — ตั้งค่า RAILWAY_TOKEN ใน Railway';
    const projectId = String(args.project_id ?? config.RAILWAY_PROJECT_ID ?? '');
    if (!projectId)
        return 'ต้องระบุ project_id หรือตั้งค่า RAILWAY_PROJECT_ID';
  ...
  `, { id: projectId }));
    ...
    if (!services.length)
        return `โปรเจค "${data.project.name}" ไม่มี service`;
...
export async function railwayRedeploy(args) {
    ...
    if (!serviceId || !environmentId)
        return 'ต้องระบุ service_id และ environment_id';
    await gql(`
...
export async function railwayGetLogs(args) {
  ...
  `, { id: deploymentId, limit: Number(args.limit ?? 50) }));
