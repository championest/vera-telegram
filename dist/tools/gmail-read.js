...
export async function gmailListUnread(args) {
    ...
    if (messages.length === 0)
        return 'ไม่มีอีเมลที่ยังไม่ได้อ่านค่ะ ✅';
    const lines = [`*อีเมลที่ยังไม่ได้อ่าน (${messages.length} ฉบับ)*\n`];
...
export async function gmailSearch(args) {
    ...
    const messages = listRes.data.messages ?? [];
    if (messages.length === 0)
        return `ไม่พบอีเมลที่ตรงกับ: "${query}"`;
    const lines = [`*ผลค้นหา: "${query}" (${messages.length} ฉบับ)*\n`];
    for (const msg of messages) {
        const detail = await gmail.users.messages.get({
            userId: 'me',
            id: msg.id,
...
export async function gmailRead(args) {
    if (!await isConnected())
        return NOT_CONNECTED;
    const auth = await getAuthedClient();
    if (!auth)
    ...
    if (!messageId)
        return 'กรุณาระบุ message_id ค่ะ';
...
export async function gmailMarkRead(args) {
    if (!await isConnected())
        return NOT_CONNECTED;
    const auth = await getAuthedClient();
    if (!auth)
...
export async function gmailTrash(args) {
    if (!await isConnected())
        return NOT_CONNECTED;
    const auth = await getAuthedClient();
    if (!auth)
