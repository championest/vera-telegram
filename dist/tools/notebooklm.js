...
async function loadCookieHeader() {
    ...
    if (!snap.exists)
        throw new Error('ไม่มี NotebookLM session — ใช้ /nbl_cookies เพื่อ setup');
    const cookies = snap.data()['cookies'] ?? [];
    if (!cookies.length)
        throw new Error('cookies ว่างเปล่า — ใช้ /nbl_cookies ใหม่');
...
async function fetchSession(cookieHeader) {
    ...
    if (!res.ok)
        throw new Error(`GET notebooklm.google.com → ${res.status}`);
    ...
    if (!snlm0e)
        throw new Error('ดึง CSRF token ไม่ได้ — cookies อาจหมดอายุ');
    return { snlm0e, bl, sid };
}
// ─── batchexecute helper ─────────────────────────────────────────────────────
async function batchExecute(cookieHeader, session, rpcId, payload, reqId = 1) {
    ...
    if (!res.ok)
        throw new Error(`batchexecute ${rpcId} → HTTP ${res.status}`);
...
export async function notebooklmCreate(args) {
    ...
    try {
        ...
        if (Array.isArray(createResp)) {
            ...
            notebookId = flat.find((v) => typeof v === 'string' && v.length > 10 && !v.includes(' ')) ?? null;
        }
        if (!notebookId) {
