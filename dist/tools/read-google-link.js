...
export function parseGoogleLink(url) {
    try {
        const u = new URL(url);
        ...
        if (fileMatch)
            return { kind: 'drive', id: fileMatch[1] };
...
async function fetchPublicExport(docId, format, gid) {
...
export async function readGoogleDoc(args) {
    const url = String(args.url ?? '').trim();
    if (!url)
        return 'กรุณาส่ง url ของ Google Doc';
    ...
    if (auth) {
        try {
            ...
            if (mimeType === 'application/vnd.google-apps.document') {
                const exportRes = await drive.files.export({ fileId: parsed.id, mimeType: 'text/plain' }, { responseType: 'text' });
            ...
            if (mimeType.startsWith('text/') || mimeType === 'application/json') {
                const dl = await drive.files.get({ fileId: parsed.id, alt: 'media' }, { responseType: 'text' });
...
export async function readGoogleSheet(args) {
    const url = String(args.url ?? '').trim();
    const argRange = args.range ? String(args.range) : undefined;
    if (!url)
        return 'กรุณาส่ง url ของ Google Sheet';
    const parsed = parseGoogleLink(url);
