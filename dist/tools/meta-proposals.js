...
export async function listProposals() {
    ...
    if (!ps.length)
        return 'ไม่มีงานค้างรออนุมัติค่ะ ✅ (meta-loop ยังไม่เจออะไรใหม่)';
    const lines = ['*งานค้างรออนุมัติ* (meta-loop)\n'];
    for (const p of ps)
        lines.push(`*${p.n}.* ${p.title}\n   \`${p.task.slice(0, 120)}\``);
    lines.push('\nสั่งรัน: `/queue 1 3` · ทิ้ง: `/dismiss 2`');
    return lines.join('\n');
}
function parseNums(arg) {
    return (arg || '').split(/[\s,]+/).map(Number).filter((n) => Number.isInteger(n) && n > 0);
}
export async function queueProposals(arg) {
    const nums = parseNums(arg);
    if (!nums.length)
        return 'ระบุเลขงานที่จะสั่งรันด้วยค่ะ เช่น `/queue 1 3`';
    const ps = await openProposals();
    const chosen = ps.filter((p) => nums.includes(p.n));
    if (!chosen.length)
        return 'ไม่พบเลขงานที่ตรงกับรายการปัจจุบันค่ะ ลอง `/proposals` ดูใหม่';
    const done = [];
...
export async function dismissProposals(arg) {
    const nums = parseNums(arg);
    if (!nums.length)
        return 'ระบุเลขงานที่จะทิ้งด้วยค่ะ เช่น `/dismiss 2`';
    const ps = await openProposals();
    const chosen = ps.filter((p) => nums.includes(p.n));
    if (!chosen.length)
        return 'ไม่พบเลขงานที่ตรงกับรายการปัจจุบันค่ะ';
    for (const p of chosen)
        await db.collection('meta-loop-proposals').doc(p.id).update({ status: 'dismissed' });
